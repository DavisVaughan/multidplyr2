...
cluster_assign_each <- function(cluster, name, values) {
  ...
  fs <- vector("list", length(cluster))
  for(ii in seq_along(cluster)) {
    fs[[ii]] <- future::futureCall(FUN = assign_silent, args = list(name = name, value = values[[ii]]))
  }
  future::values(fs) # block and resolve
